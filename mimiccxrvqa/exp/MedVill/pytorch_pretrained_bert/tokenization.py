    ...
    def from_pretrained(cls, pretrained_model_name, cache_dir=None, *inputs, **kwargs):
        ...
        try:
            ...
            logger.error(
                ...
                "associated to this path or url.".format(pretrained_model_name, ", ".join(PRETRAINED_VOCAB_ARCHIVE_MAP.keys()), vocab_file)
            )
            return None
...
def tokenize_word(merge_rules, word, dropout=0.0, random_generator=np.random.RandomState(), sentinels=["^", "$"], regime="begin", bpe_symbol="`", always_merge_sentinels=True):
