...
from lib.BAN.classifier import SimpleClassifier
from lib.network.connect import FCNet
from lib.network.connect import BCNet
from lib.network.counting import Counter
from lib.utils.utils import tfidf_loading
from lib.network.maml import SimpleCNN
from lib.network.auto_encoder import Auto_Encoder_Model
from lib.language.classify_question import typeAttention 
...
def seperate(v, q, a, att, answer_target, n_unique_close):     #q: b x 12 x 1024  v:  b x 1 x 128 answer_target : 1 x b
    ...
    return v[indexs_open,:,:].clone(), v[indexs_close,:,:].clone(), q[indexs_open,:,:].clone(), \
            q[indexs_close,:,:].clone(), a[indexs_open, n_unique_close:].clone(), a[indexs_close,:n_unique_close].clone(), \
            att[indexs_open,:].clone(), att[indexs_close,:].clone(), indexs_open, indexs_close
    ...
    def __init__(self, dataset, cfg, device):
        ...
        if cfg.TRAIN.VISION.CLIP:
            ...
            if not cfg.TRAIN.VISION.CLIP_ORG:
                checkpoint = torch.load(cfg.TRAIN.VISION.CLIP_PATH, map_location='cpu')
