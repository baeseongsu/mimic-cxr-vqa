    ...
    if use_tfidf:
        ...
        if cfg.TRAIN.QUESTION.USEDATA:
            if os.path.isfile(os.path.join(data_dir, 'embed_tfidf_weights.pkl')) == True:
                ...
                tfidf, weights = tfidf_from_questions(['train', 'valid'], cfg, dict)
...
def tfidf_from_questions(names, cfg, dictionary):
        ...
        for name in names:
            ...
            for question in questions:
                if question['q_lang'] == "en":
                    ...
                    continue
    elif 'mimiccxr' in target:
        for name in names:
            assert name in ['train', 'valid']
            # Hard coding
            question_path = os.path.join(os.path.dirname(dataroot), name + '.json')
            questions = pd.DataFrame(json.load(open(question_path))).question.values
            for question in questions:
                populate(inds, df, question)
    else:
    ...
    emb_dim = 300
    weights, word2emb = create_glove_embedding_init(dictionary.idx2word[N:], "lib/language/glove.6B.%dd.txt" % emb_dim)
    print(dictionary.idx2word[N:])
