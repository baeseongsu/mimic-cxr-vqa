...
# https://github.com/huggingface/pytorch-transformers/blob/v0.4.0/pytorch_pretrained_bert/optimization.py
...
class BertAdamFineTune(BertAdam):
    def __init__(self, params, lr=required, warmup=-1, t_total=-1, schedule="warmup_linear", b1=0.9, b2=0.999, e=1e-6, weight_decay=0.01, max_grad_norm=1.0):
        ...
        super(BertAdamFineTune, self).__init__(params, lr, warmup, t_total, schedule, b1, b2, e, weight_decay, max_grad_norm)
            ...
            for i_p, p in enumerate(group["params"]):
                ...
                if group["weight_decay"] > 0.0:
                    if self.init_param_group:
                        update += group["weight_decay"] * (2.0 * p.data - self.init_param_group[i_group][i_p])
                    else:
                        update += group["weight_decay"] * p.data
    ...
    def load_state_dict_subset_finetune(self, state_dict, num_load_group):
        ...
        # Update the state
        id_map = {old_id: p for old_id, p in zip(chain(*(g["params"] for g in saved_groups[:num_load_group])), chain(*(g["params"] for g in groups[:num_load_group])))}
...
def find_state_dict_subset_finetune(org_state_dict, org_name_list, no_decay, param_optimizer):
    ...
    org_grouped_names = [[n for n in org_name_list if not any(nd in n for nd in no_decay)], [n for n in org_name_list if any(nd in n for nd in no_decay)]]
    org_n2id, org_id2n = {}, {}
    ...
    g_np_list = [
        [(n, p) for n, p in param_optimizer if n in want_name_set and not any(nd in n for nd in no_decay)],
        [(n, p) for n, p in param_optimizer if n in want_name_set and any(nd in n for nd in no_decay)],
        [(n, p) for n, p in param_optimizer if n not in want_name_set and not any(nd in n for nd in no_decay)],
        [(n, p) for n, p in param_optimizer if n not in want_name_set and any(nd in n for nd in no_decay)],
    ]
    ...
    new_state_dict["state"] = {pid: v for pid, v in org_state_dict["state"].items() if pid not in org_id2n or org_id2n[pid] in want_name_set}
    ...
    def _filter_group(group, g_np_list, i, org_n2id):
        ...
        packed["params"] = [pid for pid in group["params"] if pid in org_id2n and org_id2n[pid] in want_name_set]
        assert len(g_np_list[i]) == len(packed["params"])
        # keep them the same order
        packed["params"] = [org_n2id[n] for n, p in g_np_list[i]]
        return packed

    new_state_dict["param_groups"] = [_filter_group(g, g_np_list, i, org_n2id) for i, g in enumerate(org_state_dict["param_groups"])]
