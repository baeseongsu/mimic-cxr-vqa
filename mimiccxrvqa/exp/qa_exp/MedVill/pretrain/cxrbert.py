...
class CXRBertEncoder(BertPreTrainedModel):
    ...
    def forward(self, cls_tok, input_txt, attn_mask, segment, input_img, sep_tok):
        ...
        if self.args.disturbing_mask:
            ...
            encoded_layers = self.encoder(encoder_input, extended_attn_mask, output_hidden_states=False)
            cls_represent = encoded_layers[-1][:, 0] * encoded_layers[-1][:, self.args.num_image_embeds + 2]  # element wisely multiplication.
