...
from trainer import CXRBERT_Trainer
...
def train(args):
    ...
    test_dataset = CXRDataset(args.test_dataset, tokenizer, args) if args.test_dataset is not None else None

    print("Creating dataloader...")
    train_data_loader = DataLoader(train_dataset, batch_size=args.batch_size, num_workers=args.num_workers, shuffle=True)
    test_data_loader = DataLoader(test_dataset, batch_size=args.batch_size, num_workers=args.num_workers, shuffle=False) if test_dataset is not None else None
...
if __name__ == "__main__":
    ...
    parser.add_argument("--train_dataset", type=str, required=True, help="train dataset for training")
    parser.add_argument("--test_dataset", type=str, required=True, help="test dataset for evaluating train set")

    output_path = "output/" + "_".join(str(datetime.now()).split())
    ...
    parser.add_argument("--itm_task", type=str, default=True, help="The model will train only itm task!! | True | False")
