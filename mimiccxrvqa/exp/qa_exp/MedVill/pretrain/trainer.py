...
class CXRBERT_Trainer:
    def __init__(self, args, train_dataloader, test_dataloader=None):
        # wandb logger
        wandb.init(
            project="vqa-pretraining",
            entity="ehr-vqg",
            name="MedViLL-pretraining",
        )
        ...
        if args.weight_load:
            ...
            self.model = CXRBERT.from_pretrained(args.pre_trained_model_path, state_dict=model_state_dict, config=config, args=args).to(self.device)
            print("resume")
            print(config)
        else:
    ...
    def train(self, epoch):
        ...
        train_data_iter = tqdm.tqdm(enumerate(self.train_data), desc=f"EP_:{epoch}", total=len(self.train_data), bar_format="{l_bar}{r_bar}")
        ...
        if self.args.mlm_task:
            print("avg mlm loss per epoch", np.mean(train_mlm_loss))
            print("avg mlm acc per epoch", round(total_mlm_correct / total_mlm_element * 100, 3))
            wandb.log({"train_mlm_loss": np.mean(train_mlm_loss)})
            wandb.log({"train_mlm_acc": round(total_mlm_correct / total_mlm_element * 100, 3)})

        if self.args.itm_task:
            print("avg itm loss per epoch", np.mean(train_itm_loss))
            print("avg itm acc per epoch", round(total_correct / total_element * 100, 3))
            wandb.log({"train_itm_loss": np.mean(train_itm_loss)})
            wandb.log({"train_itm_acc": round(total_correct / total_element * 100, 3)})

        test_data_iter = tqdm.tqdm(enumerate(self.test_data), desc=f"EP_:{epoch}", total=len(self.test_data), bar_format="{l_bar}{r_bar}")
        self.model.eval()
        with torch.no_grad():
            ...
            if self.args.mlm_task:
                print("avg mlm loss in testset", np.mean(eval_mlm_loss))
                print("avg mlm acc in testset", round(total_mlm_valid_correct / total_mlm_valid_element * 100, 3))
                wandb.log({"test_mlm_loss": np.mean(eval_mlm_loss)})
                wandb.log({"test_mlm_acc": round(total_mlm_valid_correct / total_mlm_valid_element * 100, 3)})

            if self.args.itm_task:
                print("avg itm loss in testset", np.mean(eval_itm_loss))
                print("avg itm acc in testset", round(total_valid_correct / total_valid_element * 100, 3))
                wandb.log({"test_itm_loss": np.mean(eval_itm_loss)})
                wandb.log({"test_itm_acc": round(total_valid_correct / total_valid_element * 100, 3)})
    ...
    def save(self, epoch, file_path):
        ...
        path = os.path.join(file_path, "ckpt.pkl")
        torch.save({
            'model_state_dict': self.model.module.state_dict() if isinstance(self.model, DataParallel) else self.model.state_dict(),
            'n_epoch': epoch,
            'optimizer_state_dict': self.optimizer.state_dict(),
            },
            path
        )
        print(f'{epoch} Model saved on {path}')
